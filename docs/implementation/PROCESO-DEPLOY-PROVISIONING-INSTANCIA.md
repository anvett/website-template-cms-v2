...
- **Resuelto (2026-08-12): "Vercel + static export + cPanel" deja de ser una compatibilidad dual simultánea por instancia — pasa a ser una elección exclusiva según si la instancia tiene CMS activo o no** (cierra la tensión con el manual 01, sección 11, anotada más abajo como decisión pendiente hasta hoy). Camino (a) de las dos opciones que este documento dejaba planteadas: instancias con CMS activo quedan exclusivamente en Vercel/Node hosting (dinámico, ISR real vía `revalidate` — cronograma Fase 3.1), sin `output: "export"` en su `next.config.mjs`. cPanel se reserva a instancias **sin CMS** (100% estáticas, Data en archivos `.js`, tal como el manual 01 lo definió originalmente) — motivo de negocio: cPanel se mantiene principalmente por el hosting de correo electrónico (reseller ya contratado), no como plataforma para servir contenido dinámico. No se construye el mecanismo de rebuild-on-change (b) — descartado, no diferido: agregar complejidad de infraestructura para sostener una combinación (CMS + cPanel) que en la práctica ningún cliente necesita no se justifica frente a simplemente enrutar esas instancias a Vercel. Ver `docs/decisions/decision-log.md` para el detalle completo de la decisión.
- **Consecuencia práctica para el provisioning de una instancia nueva con CMS activo:** al clonar el template base (que mantiene `output: "export"` como default, pensado para el caso más simple/barato — sitio sin CMS), quitar esa línea de `next.config.mjs` de la instancia del cliente y remover `images.unoptimized: true` si se quiere aprovechar la optimización de imágenes real de Vercel. Ver `src/lib/cms/` (cronograma Fase 3) para la capa de fetch que ya asume este modo dinámico.
